(function(){"use strict";
/* ===================================================================
   TEMA v21 — kontrol TIGA-MODE: Otomatis / Terang / Gelap.
   Bawaan  : "Otomatis" → mengikuti prefers-color-scheme sistem.
   Manual  : "Terang"/"Gelap" dikunci lewat atribut data-theme pada <html>
             dan SELALU menang atas pengaturan sistem.
   Kembali ke "Otomatis": preferensi tersimpan DIHAPUS sehingga situs
   mengikuti sistem secara langsung, termasuk saat pengaturan sistem
   berubah tanpa reload (listener matchMedia di bawah + di <head>).
   Kompatibel dgn penyimpanan versi dua-mode: nilai "light"/"dark"
   tetap terbaca; nilai lain diabaikan (= Otomatis).
   =================================================================== */
const KEY="rwp-theme";
const rootEl=document.documentElement;
const mqDark=window.matchMedia?window.matchMedia("(prefers-color-scheme: dark)"):null;
function systemDark(){return !!(mqDark&&mqDark.matches)}
function readStored(){try{const v=localStorage.getItem(KEY);return (v==="light"||v==="dark")?v:null}catch(e){return null}}
function currentMode(){return readStored()||"auto"}
function effectiveTheme(){const m=currentMode();return m==="auto"?(systemDark()?"dark":"light"):m}
function applyMode(mode,save){
  if(mode==="light"||mode==="dark"){rootEl.setAttribute("data-theme",mode);rootEl.setAttribute("data-theme-set","manual")}
  else{rootEl.removeAttribute("data-theme");rootEl.setAttribute("data-theme-set","auto")}
  if(save){try{if(mode==="auto")localStorage.removeItem(KEY);else localStorage.setItem(KEY,mode)}catch(e){}}
}
const modeBtns=Array.prototype.slice.call(document.querySelectorAll("[data-theme-mode]"));
